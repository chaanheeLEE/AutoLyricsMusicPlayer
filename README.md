...
이 앱은 로컬에서 인공지능 음성 인식 모델(Whisper)을 구동하고 오디오를 변환하기 위해 아래 외부 도구들을 필요로 합니다.
*(참고: 무설치 단일 실행 파일(`.exe`)로 빌드하여 배포하는 경우, FFmpeg 및 FFprobe는 빌드 과정에서 자동으로 패키징에 내장되므로 배포 대상 사용자는 **Python** 및 **`faster-whisper`**만 설치하면 됩니다.)*

### 1. FFmpeg 및 FFprobe 설치 (로컬 개발 환경 전용)
윈도우 환경에서 터미널(PowerShell 또는 CMD)을 열고 아래 명령어를 입력하여 설치합니다.
```bash
winget install Gyan.FFmpeg
```
설치 완료 후, 환경변수(PATH)가 시스템에 정상적으로 반영되도록 실행 중인 터미널이나 에디터를 재시작해 주십시오.
...
* 가사가 없는 곡을 로드한 뒤 Analyze 버튼을 누르면 로컬 AI 가사 전사가 백그라운드에서 실행됩니다.
...
> **주의 사항**
> 빌드된 포터블 실행 파일을 다른 사람에게 배포할 때도 해당 사용자의 PC에 **Python** 및 **`faster-whisper` 패키지**가 설치되어 있어야 분석 기능이 정상 작동합니다. (FFmpeg 및 FFprobe는 빌드 시 실행 파일 내부에 동봉되어 함께 패키징되므로 배포 대상자가 따로 설치할 필요가 없습니다.)**:
